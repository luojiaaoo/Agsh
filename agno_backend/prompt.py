...
teams_leader_summary_prompt = """# Agent最终回答指令

本次会话即将结束，你的对话历史将被删除，停止与用户的交互，请一次性地自主完成任务。
...
# 子智能调用策略
对用户请求中的任务，按照一个一个按顺序进行任务委派，直到所有任务都完成为止。每个子智能体的任务必须是明确的、具体的，并且聚焦于单一目标。避免模糊或宽泛的任务定义。
