...
def save_html_report_file(step_input: StepInput) -> StepOutput:
    ...
    with open(f'{session_dirpath}/{sanitize_filename(step_input.input)[:32]}_report.html', 'w', encoding='utf-8', errors='replace') as f:
...
def save_markdown_report_file(step_input: StepInput) -> StepOutput:
    ...
    session_dirpath = f'../downloads/{session_id}'
    os.makedirs(session_dirpath, exist_ok=True)
    with open(f'{session_dirpath}/{sanitize_filename(step_input.input)[:32]}_report.md', 'w', encoding='utf-8', errors='replace') as f:
        f.write(step_input.previous_step_content)
...
def save_ppt_report_file(step_input: StepInput) -> StepOutput:
    ...
    session_dirpath = f'../downloads/{session_id}'
    os.makedirs(session_dirpath, exist_ok=True)
    with open(f'{session_dirpath}/{sanitize_filename(step_input.input)[:32]}_report.ppt.html', 'w', encoding='utf-8', errors='replace') as f:
        f.write(step_input.previous_step_content)
    return 'PPT报告保存完成'
